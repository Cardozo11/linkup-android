...
public class ActiveOrderAdapter extends RecyclerView.Adapter<ActiveOrderAdapter.OrderViewHolder>{
    ...
    public ActiveOrderAdapter(Context mCtx, List<OrderModel> orderList) {
        this.mCtx = mCtx;
...
//        holder.gig_fUsername.setText(gig.getfUsername());
       ...
       // holder.ord_title.setText(ord.getCustomerUsername());
        ...
        else if(ord.getOrderTrackingStatus().equals("Completed")){
            holder.stateProgressBar.setCurrentStateNumber(StateProgressBar.StateNumber.valueOf("FOUR"));
        }

        holder.btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent=new Intent(mCtx,Workspace.class);
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                intent.putExtra(ord.INTENT_PURPOSE,"tracking");
